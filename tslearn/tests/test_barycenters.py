...
def test_set_weights():
    w = tslearn_cuda.not_used.tslearn.barycenters._set_weights(None, 3)
    np.testing.assert_allclose(w, np.ones((3, )))

    w = tslearn_cuda.not_used.tslearn.barycenters._set_weights([.5, .25, .25], 3)
    np.testing.assert_allclose(w, [.5, .25, .25])

    w = tslearn_cuda.not_used.tslearn.barycenters._set_weights([.5, .25, .25], 2)
...
def test_euclidean_barycenter():
    ...
    bar = tslearn_cuda.not_used.tslearn.barycenters.euclidean_barycenter(time_series)
...
def test_dba():
    ...
    euc_bar = tslearn_cuda.not_used.tslearn.barycenters.euclidean_barycenter(time_series)
    dba_bar = tslearn_cuda.not_used.tslearn.barycenters.dtw_barycenter_averaging_petitjean(
        time_series,
        max_iter=0)
    ...
    dba_bar = tslearn_cuda.not_used.tslearn.barycenters.dtw_barycenter_averaging_petitjean(
        time_series,
        max_iter=5)
    dba_bar_mm = tslearn_cuda.not_used.tslearn.barycenters.dtw_barycenter_averaging(
        time_series,
        max_iter=5)
...
def test_softdtw_barycenter():
    ...
    # Equal length, 0 iterations -> Euclidean
    euc_bar = tslearn_cuda.not_used.tslearn.barycenters.euclidean_barycenter(time_series)
    sdtw_bar = tslearn_cuda.not_used.tslearn.barycenters.softdtw_barycenter(time_series, max_iter=0)
    np.testing.assert_allclose(euc_bar, sdtw_bar)

    # Equal length, >0 iterations
    sdtw_bar = tslearn_cuda.not_used.tslearn.barycenters.softdtw_barycenter(time_series, max_iter=5)
